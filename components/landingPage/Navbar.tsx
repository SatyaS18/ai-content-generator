  ...
  return (
    <div className="sticky top-0 z-50 backdrop-blur-md bg-white/30 lg:px-[50px] px-5 flex items-center shadow justify-between w-full h-20">
      ...
      <div className="lg:hidden flex items-center">
        ...
        >
          {isOpen ? (
            <XSquare className="w-7 h-7 text-[#6941c6]" />
          ) : (
            <MenuSquare className="w-7 h-7 text-[#6941c6]" />
          )}
        </button>
      </div>
      ...
      <div className="hidden lg:flex">
        <ul className="flex items-center gap-7 text-gray-950 justify-between cursor-pointer">
          <Link href="/">
            <li className="hover:scale-105 transition-all hover:text-[#6941c6]">
              Home
            </li>
          </Link>
          <Link href="#services">
      ...
      <div className="lg:block hidden">
        <button
          ...
          className="bg-[#6941c6] rounded-md px-5 py-2 text-white transition-all hover:bg-purple-900"
        >
          Get Started
        </button>
      </div>
